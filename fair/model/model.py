...
class FairModel(object):
    ...
    def read_json(param_json):
        ...
        for param_name, param_value in data.items():
            if param_name in drop_params:
                continue

            # Multi-input nodes keep their nested structure.
            if param_name.startswith('multi'):
                model.input_multi_data(param_name, param_value)
                continue

            # All ordinary model parameters are expected to be dictionaries.
            if not isinstance(param_value, dict):
                err = f'Parameter "{param_name}" is not stored as a dictionary.'
                raise FairException(err)

            # Raw input is restored via input_raw_data().
            if 'raw' in param_value:
                model.input_raw_data(param_name, param_value['raw'])
                continue
           
            # Legacy and structured inputs are both handled by input_data().
    ...
    def export_results(self):
        """Export the calculation results as a dataframe.

        Returns
        -------
        pandas.DataFrame
            The internal result table.
        """
        return self._model_table

    def export_results_csv(self, output_path=None, sep=';', decimal=',', index=False):
        """Export calculation results as a CSV file.

        Parameters
        ----------
        output_path : str or pathlib.Path, optional
            Output file path. If omitted, the model name is used.
        sep : str, optional
            CSV separator.
        ...
        -------
        str
            The file path used for export.
        """
        if output_path is None:
            output_path = f'{self._name}.csv'

        self._model_table.to_csv(
            output_path,
            index=index,
            sep=sep,
            decimal=decimal
    ...
    def to_json(self):
        ...
        """
        # Copy only.
        # Use the original input state for JSON serialization so that
        # roundtrips preserve the user's intent rather than the fully
        # resolved internal parameterization.
        data = {**self._data_input.get_original_supplied_values()}
